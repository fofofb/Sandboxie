...
_FX NTSTATUS KphValidateCertificate()
{
	...
	Verify_CertInfo.opt_sec = 1;      // 开启高级安全沙盒功能
	Verify_CertInfo.opt_enc = 1;      // 开启加密沙盒功能
	Verify_CertInfo.opt_net = 1;      // 开启网络过滤功能
	Verify_CertInfo.opt_desk = 1;     // 开启独立桌面功能
	Verify_CertInfo.type = eCertDeveloper; // 标记证书类型为开发者
	Verify_CertInfo.level = eCertMaxLevel;  // 标记最高特权等级
	return STATUS_SUCCESS;            // 直接返回验证成功
